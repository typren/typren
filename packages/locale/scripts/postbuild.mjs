#!/usr/bin/env node
/**
 * Post-build fixups for `dist`, same approach as packages/core,
 * packages/cli, and packages/adapter-cloudfront's postbuild.mjs (see those
 * for the full rationale): extensionless relative imports in TS source need
 * real extensions for raw Node ESM, added here rather than in source
 * (source stays bundler-friendly).
 */
...
for (const entry of ["index.js", "build/index.js", "ota/index.js", "vue/index.js"]) {
  execFileSync(process.execPath, ["--input-type=module", "-e", `await import("./${DIST}/${entry}");`], {
    stdio: "inherit",
  });
}

console.log(`postbuild: ${rewrites} dist import specifiers extended, all 4 entry points load under node`);
